...
class S3Client:
    def __init__(
        ...
        # TODO we could add a locking mechanism (to avoid racing conditions during parallel work) like in here  https://github.com/PedroMTQ/helical_pdqueiros/blob/main/src/helical_pdqueiros but this is simpler in a POC
    ...
    def get_files(self, prefix: str, file_name_pattern: str = None, match_on_s3_path: bool = False) -> list[str]:
        ...
        try:
            ...
            raise RuntimeError(f'Error listing files under s3://{self.bucket_name}/{prefix} due to: {e}') from e
    ...
    def file_exists(self, s3_path: str) -> bool:
        try:
            self.__client.head_object(Bucket=self.bucket_name, Key=s3_path)
            return True
    ...
    def get_object(self, object_name: str):
        ...
        try:
            return self.__client.get_object(Bucket=self.bucket_name, Key=object_name)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    print(client.get_files(prefix='silver', file_name_pattern=r'deduplicated\.parquet'))
