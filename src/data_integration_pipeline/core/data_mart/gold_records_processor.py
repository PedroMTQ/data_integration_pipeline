...
class GoldRecordsProcessor:
    ...
    def create_gold_master(table: pa.Table):
        ...
        gold_master = df.select([
            ...
            # we exclude alt_entities because it's not needed in the gold master
    ...
    def create_bridge_data(data: pa.Table) -> pa.Table:
        ...
        print(f"{'-' * 20}{'Non-anchor entities and their respective anchors'}{'-' * 20}")
        print(alts)
        ...
        bridge = bridge.with_columns([pl.col("entity_id").cast(pl.String), pl.col("data_source").cast(pl.String), pl.col("anchor_entity_id").cast(pl.String)])
        print(f"{'-' * 20}{'Bridge table'}{'-' * 20}")
        print(bridge)


        duplicate_check = pl.concat([anchors, alts]).filter(pl.col("entity_id").is_not_null())
        print(f"Total records before unique: {len(duplicate_check)}")

        # Find the specific ID that appears in more than one cluster (The 'Ghost' match)
        clash_check = (
            duplicate_check
            .group_by(["entity_id", "data_source"])
            .agg(pl.count().alias("occurrence"))
            .filter(pl.col("occurrence") > 1)
        )
        print("Duplicate ID/Source pairs being collapsed:")
        print(clash_check)
        ...
        with S3FileReader(s3_path=s3_path, bucket_name=DATA_BUCKET, as_table=True) as reader:
            for i, batch in enumerate(reader):
                ...
                if i == 0:
                    ...
                    self.delta_client.write(s3_path=self.gold_s3_path,data=gold_chunk,
        ...
        with duckdb.connect(self.db_path) as connection:
            ...
            connection.execute(f"""
                ...
                SELECT
                    ...
                    gold.* EXCLUDE (anchor_entity_id, {HASH_DIFF_COLUMN}, {LDTS_COLUMN})
