...
class SplinkClient:
    def __init__(
        ...
        self.db_path = os.path.join(TEMP, ENTITY_RESOLUTION_DATA_FOLDER, 'splink', 'er.duckdb')
        ...
        self.model_path = os.path.join(TEMP, ENTITY_RESOLUTION_DATA_FOLDER, 'splink', 'er_model.json')
    ...
    def write_table(self, table_path: str, primary_key: str, table_name: str, schema: pa.Schema) -> str:
        ...
        with S3FileReader(s3_path=table_path, as_table=True) as reader:
    ...
    def write_links(self, links_data: Iterable[dict]) -> str:
        ...
        with S3FileWriter(s3_path=s3_path) as writer:
            for row in links_data:
                writer.write_row(row)
        logger.info(f'Wrote links to {s3_path}')
        return s3_path

    def write_metadata(self, metadata: SplinkRunMetadata) -> str:
        s3_path = os.path.join(self.s3_folder, LINKS_METADATA_FILE_NAME)
        with S3FileWriter(s3_path=s3_path) as writer:
            writer.write_json(metadata.to_dict())
    ...
    def run(self) -> SplinkRunMetadata:
        ...
        metadata: SplinkRunMetadata = self.generate_run_metadata(
            links_s3_path=links_s3_path,
            table_names=table_names,
            linker=linker,
            links_count=links_count,
        ...
        logger.info(f'Splink run results: {metadata}')
