...
from data_integration_pipeline.settings import DATA_BUCKET, ENTITY_RESOLUTION_DATA_FOLDER, TEMP
...
class DuplicatesProcessor:
    ...
    def __init__(self, partition_by_keys: list[str]):
        ...
        if not partition_by_keys:
            raise Exception('We need primary keys for deduplication!')
    ...
    def _deduplicate_integrated(self, input_path: str, output_path: str, connection: duckdb.DuckDBPyConnection) -> dict:
        ...
        connection.execute(f"""
            ...
            """)
        # TODO: we should avoid reading from s3 twice, we could instead just load the data to a temp db and the get the count from disk. Same for the other method
        initial_count: int = connection.execute(f"SELECT COUNT(*) FROM read_parquet('{input_path}')").fetchone()[0]
        deduplicated_count: int = connection.execute(f"SELECT COUNT(*) FROM read_parquet('{output_path}')").fetchone()[0]
        return {'initial_count': initial_count, 'deduplicated_count': deduplicated_count}
